...
${parseExtras(gltf.parser.json.asset && gltf.parser.json.asset.extras)}*/
        ...
        import React${options.forward ? '' : ', { useRef }'} from 'react'
        ...
        export ${
          options.forward
            ? 'const ' +
              model +
              ' = React.forwardRef' +
              (options.types ? "<THREE.Group, JSX.IntrinsicElements['group']>" : '') +
              '((props, ref) => '
            : 'function ' + model + '(props' + (options.types ? ": JSX.IntrinsicElements['group'])" : '')
        } {
          ${options.forward ? '' : 'const group = ' + (options.types ? 'useRef<THREE.Group>()' : 'useRef()')}
    ...
    options.draco ? `, ${JSON.stringify(options.draco)}` : ''
          ...
          return (
            <group ref={${options.forward ? 'ref' : 'group'}} {...props} dispose={null}>
        ${scene}
            </group>
          )
        }${options.forward ? ')' : ''}
